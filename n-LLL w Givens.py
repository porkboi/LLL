...
def GramSchmidt(A):
    #actually GivensRot but we were lazy
    ori = A.copy()
    n = len(A[0])-1
    #print(n)
    ...
    for i in range(n):
        for j in range(n, i, -1):
            mag = math.sqrt(Q[i][j-1]**2 + Q[i][j]**2)
            costheta, sintheta = Q[i][j-1]/mag, Q[i][j]/mag
            RotMat = [[costheta, sintheta], [-sintheta, costheta]]
            #RotMat2 = RotMat.copy()
            Q = np.transpose(Q)
            Q[j-1], Q[j] = np.dot(RotMat, [Q[j-1], Q[j]])
            Q = np.transpose(Q)
            R = np.transpose(R)
            R[j-1], R[j] = np.dot(RotMat, [R[j-1], R[j]])
            R = np.transpose(R)
    Q = Q.tolist()
    R = np.transpose(R).tolist()
    for i in range(n+1):
        R[i] = np.multiply(Q[i][i], R[i]).tolist()
    mulist = []
    for i in range(n+1):
        minimulist = []
        for j in range(i):
            mu = Q[i][j] / Q[j][j]
            minimulist.append(mu)
        mulist.append(minimulist)
    mulist.pop(0)
    mulist.insert(0, [0])
    return (ori, R, mulist)
